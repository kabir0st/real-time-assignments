...
void spawnWithDeadline(void (* function)(int), int arg, unsigned int deadline, unsigned int rel_deadline) {
    thread newp;
    DISABLE();
    if (!initialized) initializeThreads();

    newp = dequeue(&freeQ);
    if (newp == NULL) {
        print2uart("spawnWithDeadline: no free thread blocks\n");
        ENABLE();
        return;
    }
    ...
    newp->function = function;
    newp->arg = arg;
    // absolute activation time: next activation = now + deadline
    newp->Period_Deadline = ticks + deadline;
    newp->Rel_Period_Deadline = rel_deadline;
    newp->next = NULL;

    if (setjmp(newp->context) == 1) {
        // when this thread is scheduled and runs to completion:
        ENABLE();
        current->function(current->arg);
        DISABLE();
        ...
        if (current->Rel_Period_Deadline != INT_MAX) {
            ...
            // non-periodic fallback (shouldn't happen for spawnWithDeadline)
            enqueue(current, &freeQ);
        }
        dispatch(dequeue(&readyQ));
    }
    ...
    SETSTACK(&newp->context, &newp->stack);
    // insert into readyQ (readyQ is kept sorted by Rel_Period_Deadline)
    enqueue(newp, &readyQ);
    ENABLE();
}
